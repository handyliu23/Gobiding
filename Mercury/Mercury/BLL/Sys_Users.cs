...
namespace Mercury.BLL
{
	...
	{
		private readonly Mercury.DAL.Sys_Users dal=new Mercury.DAL.Sys_Users();
		public Sys_Users()
		{}
		#region  BasicMethod
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Mercury.Model.Sys_Users GetModelByCache(int Sys_UserId)
		{
			...
			return (Mercury.Model.Sys_Users)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Mercury.Model.Sys_Users> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Mercury.Model.Sys_Users> DataTableToList(DataTable dt)
		{
			List<Mercury.Model.Sys_Users> modelList = new List<Mercury.Model.Sys_Users>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Mercury.Model.Sys_Users model;
