...
namespace GoBiding.BLL
{
	...
	{
		private readonly GoBiding.DAL.SystemLog dal=new GoBiding.DAL.SystemLog();
		public SystemLog()
		{}
		#region  BasicMethod
		/// <summary>
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public GoBiding.Model.SystemLog GetModelByCache(long SystemLogId)
		{
			...
			return (GoBiding.Model.SystemLog)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<GoBiding.Model.SystemLog> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<GoBiding.Model.SystemLog> DataTableToList(DataTable dt)
		{
			List<GoBiding.Model.SystemLog> modelList = new List<GoBiding.Model.SystemLog>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				GoBiding.Model.SystemLog model;
