...
(function () {
    ...
    function c(u) {
        var v = u.children,
		w, x, y = u.children.length,
		z, A, B = /list-style-type:(.*?)(?:;|$)/,
    ...
    var p = 0,
	q = null,
	r, s = CKEDITOR.plugins.pastefromword = {
	    utils: {
	        ...
	        resolveList: function (u) {
	            ...
	            if ((w = u.removeAnyChildWithName('cke:listbullet')) && w.length && (w = w[0])) {
	                u.name = 'cke:li';
	                if (v.style) v.style = s.filters.stylesFilter([['text-indent'], ['line-height'], [/^margin(:?-left)?$/, null,
					function (x) {
					    var y = x.split(' ');
	    ...
	    filters: {
	        flattenList: function (u, v) {
	            ...
	            for (var A = 0; A < y.length; A++) {
	                ...
	                if (z.name in CKEDITOR.dtd.$listItem) {
	                    ...
	                    s.filters.stylesFilter([['tab-stops', null,
						function (H) {
	        ...
	        stylesFilter: function (u, v) {
	            return function (w, x) {
	                var y = []; (w || '').replace(/&quot;/g, '"').replace(/\s*([^ :;]+)\s*:\s*([^;]+)\s*(?=;|$)/g,
					function (A, B, C) {
					    B = B.toLowerCase();
	    ...
	    getRules: function (u) {
			...
			v.$block, v.$listItem, v.$tableContent),
			x = u.config,
			y = this.filters,
			z = y.falsyFilter,
			A = y.stylesFilter,
	        ...
	        return {
	                    ...
	                    if (CKEDITOR.env.gecko) {
	                        ...
	                        if (Q) {
								...
								function (S, T, U) {
								    ...
								    var V = T.length,
									W;
								    for (var X = 0; X < V; X++) CKEDITOR.tools.trim(T[X]).replace(/^(\w+)(\.[\w-]+)?$/g,
									function (Y, Z, aa) {
									    Z = Z || '*';
				...
				function (O, P) {
				    ...
				    if (CKEDITOR.env.gecko && Q) {
				        var U = CKEDITOR.htmlParser.fragment.fromHtml(Q[0]).children[0],
						V = P.previous,
						W = V && V.value.match(/<v:imagedata[^>]*o:href=['"](.*?)['"]/),
						X = W && W[1];
